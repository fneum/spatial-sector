...
JRC-IDEES database.\citeS{europeancommission.jointresearchcentre.JRCIDEESIntegrated2017}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{\textwidth}
        ...
        \includegraphics[width=.8\textwidth, trim=0cm 1cm 0cm 0cm, clip]{bev-availability-month}
    \end{subfigure}
    \vspace{-0.5cm}
    \begin{subfigure}[t]{\textwidth}
        \centering
        \caption{availability (per-unit)}
        \includegraphics[width=.8\textwidth, trim=0cm 1cm 0cm 0cm, clip]{bev-demand-month}
    \end{subfigure}
...
\SI{0.7}{\kwh\per\kilo\metre}\citeS{europeancommission.jointresearchcentre.JRCIDEESIntegrated2017}) to the
...
Research Institute (BASt)\citeS{bundesanstaltfurstrassenwesenAutomatischeZahlstellen2021} are used to generate hourly time series for
...
The shipping sector consumes synthetic methanol.
% TODO We include a sensitivity analysis where liquid hydrogen is used in shipping. 
For international shipping, the demand per country is regionally distributed by
port trade volumes taken from the World Bank Data Catalog\cite{}. For domestic shipping, the demand is
distributed by population. Other fuel options like ammonia are currently not
considered.
% TODO add citation https://datacatalog.worldbank.org/search/dataset/0038118/Global---International-Ports